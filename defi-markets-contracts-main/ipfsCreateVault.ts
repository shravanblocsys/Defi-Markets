...
/**
 * Upload metadata JSON to IPFS using IPFS HTTP API, Pinata, or NFT.Storage
 * 
...
async function uploadMetadataToIPFS(
  ...
  // Option 1: Using IPFS HTTP API (ipfs.tech or local node)
  // Set IPFS_API_URL environment variable (e.g., "http://localhost:5001" or "https://ipfs.tech")
  const IPFS_API_URL = process.env.IPFS_API_URL || process.env.IPFS_NODE_URL;
  
  if (IPFS_API_URL) {
    try {
      console.log(`📤 Uploading to IPFS node: ${IPFS_API_URL}`);
      
      // For Node.js, use form-data package
      ...
      const apiUrl = IPFS_API_URL.endsWith('/') 
      ...
      // IPFS API returns: { "Name": "metadata.json", "Hash": "Qm...", "Size": "123" }
      ...
      try {
        const pinUrl = IPFS_API_URL.endsWith('/') 
          ? `${IPFS_API_URL}api/v0/pin/add?arg=${ipfsHash}` 
          : `${IPFS_API_URL}/api/v0/pin/add?arg=${ipfsHash}`;
        await fetch(pinUrl, { method: 'POST' });
        console.log("📌 File pinned to IPFS node");
      } catch (pinError) {
        console.warn("⚠️ Could not pin file (may need to pin manually):", pinError);
      }
      
      return ipfsUri;
    } catch (error) {
      console.error("❌ IPFS HTTP API upload error:", error);
      throw error;
    }
  }

  // Option 2: Using Pinata (requires PINATA_JWT_TOKEN environment variable)
  ...
  // Option 3: Using NFT.Storage (requires NFT_STORAGE_API_KEY environment variable)
  ...
  console.warn("⚠️ No IPFS service configured. Set one of the following:");
  console.warn("   - IPFS_API_URL (for IPFS HTTP API, e.g., http://localhost:5001 or https://ipfs.tech)");
  console.warn("   - PINATA_JWT_TOKEN (or PINATA_JWT) for Pinata");
  console.warn("   - NFT_STORAGE_API_KEY for NFT.Storage");
...
async function uploadLogoToIPFS(imagePath: string): Promise<string> {
    ...
    try {
      ...
      try {
        const pinUrl = IPFS_API_URL.endsWith('/') 
          ...
          : `${IPFS_API_URL}/api/v0/pin/add?arg=${ipfsHash}`;
        await fetch(pinUrl, { method: 'POST' });
        console.log("📌 Logo pinned to IPFS node");
      } catch (pinError) {
        console.warn("⚠️ Could not pin logo:", pinError);
      }
      
      return ipfsUrl;
    } catch (error) {
      console.error("❌ IPFS logo upload error:", error);
      throw error;
    }
  }

  // Option 2: Using Pinata (fallback)
  const PINATA_JWT = process.env.PINATA_JWT_TOKEN || process.env.PINATA_JWT;
  ...
  throw new Error("No IPFS service configured. Set IPFS_API_URL or PINATA_JWT_TOKEN.");
}
...
async function createVault(factoryPDA: PublicKey) {
  ...
  try {
    ...
    const DEFAULT_LOGO_URL = `ipfs://${DEFAULT_LOGO_IPFS_CID}`;
    const logoUrl = process.env.VAULT_LOGO_URL || DEFAULT_LOGO_URL;
