...
import { PublicKey, SystemProgram, Connection, Keypair, SYSVAR_RENT_PUBKEY, Transaction, VersionedTransaction } from "@solana/web3.js";
...
const WALLET_ADDRESS = "D8RxevzCjfJPqfR6gAowwzg997AV44cNgWHCw2nmRqbx";
...
const DERIVATION_PATH = "44'/501'/0'/0'"; // Standard Solana derivation path
let ledgerTransport: any = null;
let solanaApp: SolanaApp | null = null;
let ledgerPublicKey: PublicKey | null = null;

// Ledger Wallet class that implements Anchor Wallet interface
class LedgerWallet {
  publicKey: PublicKey;
  private signTx: <T extends Transaction | VersionedTransaction>(tx: T) => Promise<T>;
  private signAllTxs: <T extends Transaction | VersionedTransaction>(txs: T[]) => Promise<T[]>;
  ...
  constructor(
    ...
    this.signTx = signTransaction;
    this.signAllTxs = signAllTransactions;
  }
  ...
  async signAllTransactions<T extends Transaction | VersionedTransaction>(txs: T[]): Promise<T[]> {
    return this.signAllTxs(txs);
  }
}

// Initialize Ledger connection
async function initializeLedger(): Promise<{ publicKey: PublicKey; wallet: LedgerWallet }> {
  console.log("📱 Connecting to Ledger device...");
  console.log("   Please make sure:");
  console.log("   1. Ledger is connected via USB");
  console.log("   2. Ledger is unlocked");
  console.log("   3. Solana app is open on Ledger");
  console.log("   4. Ledger Live is closed (it blocks CLI access)\n");

  try {
    // Connect to Ledger
    ledgerTransport = await TransportNodeHid.create();
    solanaApp = new SolanaApp(ledgerTransport);
    ...
    const signTransaction = async <T extends Transaction | VersionedTransaction>(tx: T): Promise<T> => {
      ...
      if (tx instanceof VersionedTransaction) {
        // For VersionedTransaction, sign the message
        const message = tx.message.serialize();
        const signature = await solanaApp.signTransaction(DERIVATION_PATH, Buffer.from(message));
        tx.addSignature(ledgerPublicKey!, Buffer.from(signature.signature));
      } else {
        // For legacy Transaction
        const message = tx.serializeMessage();
        const signature = await solanaApp.signTransaction(DERIVATION_PATH, message);
        tx.addSignature(ledgerPublicKey!, Buffer.from(signature.signature));
      }
      
      console.log("✅ Transaction signed by Ledger\n");
      return tx;
    };
    ...
    return { publicKey: ledgerPublicKey, wallet };
  } catch (error: any) {
...
// Cleanup function to close Ledger connection
...
// Global variables for Ledger wallet and provider
let wallet: LedgerWallet;
let provider: anchor.AnchorProvider;
let connection: Connection;
let program: anchor.Program<VaultMvp>;

// Provider + connection
const RPC_URL = "https://api.mainnet-beta.solana.com";
const programId = new PublicKey("CtH2WicL6g2NkP3GPzc6aKyvkeeHxioiaiG8kXjCaHVs");

// Helper function to create a Ledger signer for SPL token operations
// SPL token operations need a signer interface, so we create a wrapper
function createLedgerSigner(publicKey: PublicKey) {
  return {
    publicKey: publicKey,
    // Add a dummy secretKey for SPL token library compatibility
    // This won't be used since we're signing with Ledger
    secretKey: new Uint8Array(64),
    signTransaction: async (tx: Transaction): Promise<Transaction> => {
      if (!solanaApp || !ledgerPublicKey) {
        throw new Error("Ledger not connected");
      }
      console.log("✍️  Signing transaction with Ledger...");
      console.log("   Please review and approve on your Ledger device...");
    ...
    signAllTransactions: async (txs: Transaction[]): Promise<Transaction[]> => {
      ...
      for (const tx of txs) {
        const signed = await createLedgerSigner(publicKey).signTransaction(tx);
        signedTxs.push(signed);
      }
      return signedTxs;
    }
  } as any; // Type assertion to bypass strict type checking for SPL token operations
}
...
async function createStablecoinToken() {
  ...
  const ledgerSigner = createLedgerSigner(ledgerPublicKey);
  ...
  try {
    ...
    const mint = await createMint(
      connection,
      ledgerSigner, // payer
      ledgerPublicKey, // mint authority
      ledgerPublicKey, // freeze authority
      6 // decimals
    );
...
async function updateFactoryFees(
  ...
  try {
    ...
    console.log("👤 Admin:", ledgerPublicKey.toBase58());
...
async function distributeAccruedFees(factoryPDA: PublicKey, vaultIndex: number) {
  ...
  try {
    ...
    const vaultAdminVaultAccount = await getOrCreateAssociatedTokenAccount(
      ...
      ledgerSigner, // payer
      vaultMintPDA, // mint
      ledgerPublicKey // owner (vault admin)
    );
    ...
    const feeRecipientVaultAccount = await getOrCreateAssociatedTokenAccount(
      ...
      ledgerSigner, // payer
      vaultMintPDA, // mint
      ledgerPublicKey // owner (fee recipient - same as admin for now)
    );
...
(async () => {
  ...
  console.log("📱 Using Ledger wallet for signing\n");

  try {
    // Initialize Ledger connection
    const { publicKey: ledgerPubKey, wallet: ledgerWallet } = await initializeLedger();
    
    // Setup connection and provider
    connection = new Connection(RPC_URL, "confirmed");
    provider = new anchor.AnchorProvider(connection, ledgerWallet as any, {
      preflightCommitment: "processed"
    });
    anchor.setProvider(provider);

    // Create program instance using the actual IDL
    program = new anchor.Program<VaultMvp>(
      idl as anchor.Idl,
      provider
    ) as anchor.Program<VaultMvp>;

    console.log("✅ Ledger wallet initialized");
    console.log("📋 Wallet Address:", ledgerPubKey.toBase58());
