...
export class VaultFeesCalculationService {
  ...
  private initializeSolanaConnection(): void {
    ...
    const factoryAddress = this.configService.get('SOLANA_VAULT_FACTORY_ADDRESS');
    if (!factoryAddress) {
      throw new Error('SOLANA_VAULT_FACTORY_ADDRESS is required in environment variables');
    }
  ...
  private async getTokenDecimals(mintAddress: string): Promise<number> {
    try {
      ...
      throw new Error(`Cannot determine token decimals for ${mintAddress}. This is required for accurate calculations. Error: ${error.message}`);
    }
  }

  private pdaFactory(): PublicKey {
    return PublicKey.findProgramAddressSync([Buffer.from('factory_v2')], this.program.programId)[0];
  }
  ...
  private async fetchJupiterPrices(mintAddresses: PublicKey[]): Promise<Record<string, number>> {
    if (mintAddresses.length === 0) return {};
    
    const ids = mintAddresses.map(m => m.toBase58()).join(',');
    const url = `${this.JUP_PRICE_API}${ids}`;
    
    this.logger.log(`🌐 Fetching live prices from Jupiter for ${mintAddresses.length} tokens...`);
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (response.status === 429) {
          ...
          this.logger.log(`Server responded with 429 Too Many Requests. Retrying after ${delay}ms delay...`);
        ...
        this.logger.log(`Error fetching prices, retrying after ${delay}ms:`, error.message);
        await new Promise(resolve => setTimeout(resolve, delay));
        retryCount++;
      }
    }
  ...
  async calculateVaultFees(vaultIndex: number): Promise<{
    ...
    try {
      ...
      const vaultAccount = await (this.program.account as any).vault.fetch(vault) as any;
      
      // Get vault details from database
      const dbVault = await this.vaultFactoryService.findAll().then(vaults => 
      ...
      const contractAccruedFees = Number(vaultAccount.accruedManagementFeesUsdc || 0); // Already in 6-decimal USD
      ...
      let allMintAddresses = [...underlyingAssets.map((a: any) => a.mintAddress)];
      
      try {
        const stableBalance = await getAccount(this.connection, vaultStable);
        allMintAddresses.push(stableBalance.mint);
      } catch (e) {
        this.logger.warn(`Stablecoin account not found for vault ${vaultIndex}, skipping stablecoin price fetch`);
      }
      ...
      try {
        ...
        if (!stablecoinPrice) {
          this.logger.warn(`No price available for stablecoin ${stablecoinMint}`);
        } else {
          // Convert raw balance to human-readable format, then to USD using live price
          const balanceInTokens = Number(stableBalance.amount) / Math.pow(10, stablecoinDecimals);
          const valueUsd = balanceInTokens * stablecoinPrice; // Use live USDC price from Jupiter
          liveGav += valueUsd; // Store in base10 format
          this.logger.log(`  Stablecoin ${stablecoinMint}: ${balanceInTokens.toFixed(6)} tokens (${stablecoinDecimals} decimals) = $${valueUsd.toFixed(6)}`);
        }
      } catch (e) {
        ...
        this.logger.warn(`Stablecoin balance not found for vault ${vaultIndex}: ${e.message}`);
      }
      ...
      for (let i = 0; i < underlyingAssets.length; i++) {
        ...
        try {
          ...
          const balanceInTokens = Number(balance.amount) / Math.pow(10, tokenDecimals);
          const valueUsd = balanceInTokens * priceUsd;
          liveGav += valueUsd; // Store in base10 format
          this.logger.log(`  Asset ${mintAddress}: ${balanceInTokens.toFixed(6)} tokens (${tokenDecimals} decimals) = $${valueUsd.toFixed(6)}`);
        } catch (e) {
          if (e.message.includes('Cannot determine token decimals')) {
            this.logger.error(`Critical error for asset ${mintAddress}: ${e.message}`);
            throw e; // Re-throw critical errors that affect calculation accuracy
          }
          this.logger.warn(`Token account not found for ${mintAddress} in vault ${vaultIndex}: ${e.message}`);
        }
      }
      ...
      this.logger.log(`  Contract Total Supply: ${(contractTotalSupply / 1e9).toFixed(9)} (9-decimal tokens)`);
      this.logger.log(`  Contract Accrued Fees: $${(contractAccruedFees / 1_000_000).toFixed(6)} (6-decimal USD)`);
      this.logger.log(`  Live GAV (from raw balances): $${liveGav.toFixed(6)} (base10 USD)`);
      this.logger.log(`  Live NAV: $${liveNav.toFixed(6)} (base10 USD)`);
      this.logger.log(`  ETF Creator Fee: $${etfCreatorFee.toFixed(6)} (base10 USD)`);
      this.logger.log(`  Platform Owner Fee: $${platformOwnerFee.toFixed(6)} (base10 USD)`);
      ...
      this.logger.error(`Error calculating fees for vault ${vaultIndex}:`, error);
      throw error;
    }
  }
  ...
  async calculateAllVaultFees(): Promise<void> {
    ...
    if (this.isCalculating) {
      this.logger.log('⏳ Vault fees calculation already in progress, skipping...');
      return;
    }
    ...
    try {
      ...
      this.logger.log('📋 Vault details:', allVaults.map(v => ({ name: v.vaultName, index: v.vaultIndex })));
      ...
      for (const vault of allVaults) {
        ...
        if (vault.vaultIndex === null || vault.vaultIndex === undefined) {
          this.logger.log(`❌ Skipping vault ${vault.vaultName} - no vault index`);
          this.logger.warn(`Skipping vault ${vault.vaultName} - no vault index`);
          continue;
        }
        
        try {
          // Check if fees already calculated for today
          this.logger.log(`🔍 Step 2: Checking existing fees for vault ${vault.vaultName}...`);
          const existingFees = await this.vaultManagementFeesService.findByVaultIndex(vault.vaultIndex);
          this.logger.log(`📊 Found ${existingFees.length} existing fee records for vault ${vault.vaultName}`);
          
          const todayFees = existingFees.find(fee => fee.date === today);
          this.logger.log(`🔍 Checking for today's fees (${today}):`, todayFees ? 'FOUND' : 'NOT FOUND');
          
          if (todayFees) {
            this.logger.log(`⏭️ Fees already calculated for vault ${vault.vaultName} on ${today}, skipping...`);
            continue;
          }
          
          // Calculate fees
          this.logger.log(`🧮 Step 3: Calculating fees for vault ${vault.vaultName}...`);
          const feeData = await this.calculateVaultFees(vault.vaultIndex);
          this.logger.log(`✅ Fee calculation completed for vault ${vault.vaultName}`);
          
          // Store in database
          this.logger.log(`💾 Step 4: Attempting to store fees for vault ${vault.vaultName}...`);
          ...
          this.logger.log(`📝 Create data object:`, JSON.stringify(createData, null, 2));
          
          const createdFee = await this.vaultManagementFeesService.create(createData);
          this.logger.log(`✅ Successfully stored fees for vault ${vault.vaultName}`);
          ...
          this.logger.error(`Failed to calculate or store fees for vault ${vault.vaultName}:`, error);
